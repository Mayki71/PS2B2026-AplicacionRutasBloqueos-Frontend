...
import type { TipoBloqueo, Reporte } from '../reports.types';
...
export default function ReportForm({ coordsA, coordsB, initialDireccion, reportes, onClose, onSuccess }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    if (reportes) {
      const isDuplicate = reportes.some(r => {
        if (!r.ubicaciones) return false;
        // Solo bloqueos activos (asumiendo que los reportes recibidos son los vigentes)
        const dist = getDistanceInMeters(coordsA[1], coordsA[0], r.ubicaciones.latitud, r.ubicaciones.longitud);
        return dist < 50; // 50 metros de tolerancia
      });
      if (isDuplicate) {
        setLocalError('Ya existe un reporte de bloqueo en esta ubicación (a menos de 50m). Por favor, comenta o apoya el reporte existente.');
        return;
      }
    }

    // Leer id_usuario del localStorage (mantenido del original)
    ...
    await crearReporte({
      id_usuario, // Preservado de la lógica original
        ...
        <div className="rf2-body">
          ...
          {step === 4 && (
            <>
              ...
              {(error || localError) && <p className="rf2-error">{localError || error}</p>}
            </>
          )}
        </div>
